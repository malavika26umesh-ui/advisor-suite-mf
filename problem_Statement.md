Capstone Project: Mutual Fund Advisor Intelligence Suite
Objective
Build a voice-first AI mutual fund support assistant that can answer factual scheme queries, analyse customer feedback to generate actionable product insights, book advisor appointments via voice, and connect all three into a single unified dashboard.
...
A review CSV being processed into a Weekly Pulse and Fee Explainer
The Pulse Top Theme dynamically appearing in the Voice Agent greeting
A voice call completed with Booking Code generated
The Approval Centre showing three pending MCP actions being approved
The FAQ engine answering a multi-part question with source citations
At least one eval running live
Git repository with:

README (architecture + how the three pillars connect + setup steps)
List of MCP tools used
Source manifest (30+ official URLs with metadata)
How to run evals
Sample Q&A pairs and a voice transcript
Scope Constraints
To keep this achievable:

Scope your FAQ corpus to one AMC and 3–5 schemes only
Reviews CSV: 30–50 entries is sufficient
Voice scheduler: mock calendar slots are acceptable
Transit / meeting time estimates can be heuristic
Focus on quality of integration, not breadth of coverage